...
void main() {
  ...
  group('GET requests', () {
    ...
    test('404', () {
      ...
      _responseToString(request).then(expectAsync((requestContent) {
...
void testByteContent(String path) {
  test('/$path', () {
    path = "$baseUrl/$path";
    _getAsBytes(path).then(expectAsync((List contentAsBytes) {
      String content = CryptoUtils.bytesToBase64(contentAsBytes);
      Request request = _getRequest(path);
      _responseToBase64(request).then(expectAsync((requestContent) {
...
void testStringContent(String path) {
  test('/$path', () {
    path = "$baseUrl/$path";
    _getAsString(path).then(expectAsync((content) {
      Request request = _getRequest(path);
      _responseToString(request).then(expectAsync((requestContent) {
...
Future<String> _responseToBase64(Request request) {
  ...
  return response.processRequest(request, excludeHeader: true).then(
      (chrome.ArrayBuffer buffer) {
