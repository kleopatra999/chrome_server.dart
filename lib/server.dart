...
class TcpServer {
  ...
  Future<int> listen() {
    ...
    Future createFtr = properties != null ? chrome.sockets.tcpServer.create(
        properties) : chrome.sockets.tcpServer.create();

    return createFtr.then((chrome.CreateInfo createInfo) {
      this.socketId = createInfo.socketId;

      return chrome.sockets.tcpServer.listen(createInfo.socketId, this.addr,
          this.port, 1000).then((int result) {
  ...
  Future<int> send(int clientSocketId, chrome.ArrayBuffer dataToSend) {
    return chrome.sockets.tcp.send(clientSocketId, dataToSend).then(
        (chrome.SendInfo sendInfo) {
